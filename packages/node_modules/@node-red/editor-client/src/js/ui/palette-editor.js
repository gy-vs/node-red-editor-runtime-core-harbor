...
RED.palette.editor = (function() {
    ...
    // A module may provide nodes, plugins, or both. The editor loads the two
    // lists independently (GET /nodes into RED.nodes.registry and GET /plugins
    // into RED.plugins), so a module can be known to one store only, or to both.
    // This returns the merged view of a module from the two stores without
    // mutating either source. Returns undefined while the module is unknown to
    // both stores, so callers can keep/refresh any previously known state.
    function getModuleInfo(module) {
        var nodeInfo = RED.nodes.registry.getModule(module);
        var pluginInfo = RED.plugins.getModule(module);
        if (!nodeInfo && !pluginInfo) {
            return undefined;
        }
        // Build a fresh merged object; the stores themselves stay untouched.
        // Prefer metadata from the node view, but surface pending_version from
        // either store as they describe the same installed module.
        var info = nodeInfo || pluginInfo;
        var base = {
            id: module,
            name: info.name,
            version: info.version,
            local: info.local,
            pending_version: (nodeInfo && nodeInfo.pending_version) ||
                              (pluginInfo && pluginInfo.pending_version),
            sets: {},
            // `plugin` denotes a plugin-only module; a module that also
            // provides nodes is rendered as a node module with its plugins
            // listed alongside its node sets.
            plugin: !nodeInfo && !!pluginInfo
        };
        var index = [module];
        function addSets(source) {
            if (source && source.sets) {
                for (var setName in source.sets) {
                    if (source.sets.hasOwnProperty(setName)) {
                        var set = source.sets[setName];
                        base.sets[setName] = set;
                        index.push(setName);
                        if (set.types) {
                            index = index.concat(set.types);
                        }
                        if (set.plugins) {
                            for (var i=0;i<set.plugins.length;i++) {
                                if (set.plugins[i] && set.plugins[i].id) {
                                    index.push(set.plugins[i].id);
                                }
                            }
                        }
                    }
        ...
        addSets(nodeInfo);
        addSets(pluginInfo);
        base.index = index.join(",").toLowerCase();
        return base;
    }
    ...
    function _refreshNodeModule(module) {
        ...
        if (!moduleInfo) {
            // The module is not in either store. Leave any existing entry
            // untouched so its state remains traceable until a removal event
            // explicitly drops it.
            return;
        }
        if (!nodeEntries.hasOwnProperty(module)) {
            nodeEntries[module] = {info:moduleInfo};
            nodeEntries[module].index = moduleInfo.index;
            nodeList.editableList('addItem', nodeEntries[module]);
            return;
        }
        // Refresh the merged view in place so the existing editableList row
        // and its closures keep operating on current data from both stores.
        nodeEntries[module].info = moduleInfo;
        nodeEntries[module].index = moduleInfo.index;
        ...
        if (nodeEntry) {
            if (moduleInfo.plugin) {
                ...
                // Node (or node+plugin) module: keep the enable/disable control
                // visible; a previous render may have hidden it if the module was
                // temporarily only known as a plugin-only module.
                ...
                for (var setName in moduleInfo.sets) {
                    if (moduleInfo.sets.hasOwnProperty(setName)) {
                        ...
                        if (set.types) {
                            // A node set: contributes node types and can be
                            // enabled/disabled individually.
                            if (set.enabled) {
                                activeTypeCount += set.types.length;
                            }
                            typeCount += set.types.length;
                            for (var i=0;i<set.types.length;i++) {
                                var t = set.types[i];
                        ...
                        if (set.plugins) {
                            // A plugin set within a module that also provides
                            // nodes: contributes plugins to the expanded list
                            // but has no enable/disable state of its own.
                            pluginCount += set.plugins.length;
                        }
                        nodeEntries[module].setUseCount[setName] = inUseCount;
                        nodeEntries[module].totalUseCount += inUseCount;

                        if (setElements && setElements.enableButton) {
                            if (inUseCount > 0) {
                ...
                countLabel = RED._('palette.editor.nodeCount',{count:typeCount,label:nodeCount});
                if (pluginCount > 0) {
                    // Module provides both nodes and plugins: show both counts
                    countLabel = countLabel+"  +  "+RED._('palette.editor.pluginCount',{count:pluginCount,label:pluginCount});
                }
                nodeEntry.setCount.text(countLabel);
    ...
    function init() {
        ...
        RED.events.on('registry:node-set-removed', function(ns) {
            // The module may still exist as a plugin-only module after its last
            // node set is removed, so consult the merged view before dropping
            // its palette entry.
            var module = getModuleInfo(ns.module);
            if (!module) {
                ...
                _refreshNodeModule(ns.module);
            }
        });
        RED.events.on("registry:plugin-module-removed", function(module) {
            // A plugin set was removed. The module may still provide nodes, so
            // consult the merged view before dropping the palette entry.
            if (!getModuleInfo(module)) {
                var entry = nodeEntries[module];
                if (entry) {
                    nodeList.editableList('removeItem', entry);
                    delete nodeEntries[module];
                }
            } else {
                _refreshNodeModule(module);
            }
        ...
        RED.events.on("registry:plugin-module-added", function(module) {
            // Refresh via the merged view so a module that provides both
            // nodes and plugins keeps a single entry listing both. Use the
            // same debounced path as node events: the two lists load
            // independently (GET /plugins then GET /nodes), so their events can
            // arrive in quick succession and must be coalesced into one render.
                    ...
                    click: function() {
                        ...
                        removeNodeModule(entry.name, function(xhr) {
                            ...
                            if (xhr) {
                                ...
                                // Resolve the current merged view as the
                                // captured `entry` may have been refreshed since
                                // the row was first rendered.
                                const currentEntry = (nodeEntries[entry.name] && nodeEntries[entry.name].info) || entry;
                                if (currentEntry.plugin) {
                                    ...
                                    let keys = Object.keys(currentEntry.sets);
                                    keys.forEach((key) => {
                                        let set = currentEntry.sets[key];
