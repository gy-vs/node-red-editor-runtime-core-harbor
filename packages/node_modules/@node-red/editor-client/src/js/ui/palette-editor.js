...
RED.palette.editor = (function() {
    ...
    function getModuleInfo(module) {
        // A single module may provide both node sets (GET /nodes) and plugin
        // sets (GET /plugins). The two lists are held in separate registries
        // and arrive independently - including over live-update notifications.
        // Merge them here so a module's entry in the palette manager always
        // reflects both kinds of sets rather than whichever was seen first.
        var nodeModule = RED.nodes.registry.getModule(module);
        var pluginModule = RED.plugins.getModule(module);
        var base = nodeModule || pluginModule;
        if (!base) {
            return null;
        }
        var info = {
            ...
            local: base.local,
            user: base.user,
            pending_version: base.pending_version,
            // Keep references to the live registry objects so updates made by
            // either registry (enable/disable, add/remove) remain visible.
            sets: Object.assign({},
                nodeModule ? nodeModule.sets : null,
                pluginModule ? pluginModule.sets : null
            )
        };
        info.hasNodes = !!(nodeModule && Object.keys(nodeModule.sets).length > 0);
        info.hasPlugins = !!(pluginModule && Object.keys(pluginModule.sets).length > 0);
        return info;
    }

    function isPluginSet(set) {
        return !set.types && !!set.plugins;
    }

    function _refreshNodeModule(module) {
        var info = getModuleInfo(module);
        if (!info) {
            // Neither registry knows this module - nothing to show or refresh.
            return;
        }
        if (!nodeEntries.hasOwnProperty(module)) {
            nodeEntries[module] = {info:info};
            var index = [module];
            for (var s in info.sets) {
                if (info.sets.hasOwnProperty(s)) {
                    index.push(s);
                    var set = info.sets[s];
                    if (set.types) {
                    ...
                    if (set.plugins) {
                        index = index.concat(set.plugins.map(function(p) { return p.id; }));
                    }
                }
            }
            ...
            // Update the merged info in place so an expanded entry that was
            // rendered from one registry picks up sets from the other.
            nodeEntries[module].info = info;
            var moduleInfo = info;
            ...
            if (nodeEntry) {
                ...
                for (var setName in moduleInfo.sets) {
                    if (moduleInfo.sets.hasOwnProperty(setName)) {
                        ...
                        if (isPluginSet(set)) {
                            // Plugin sets can't be enabled/disabled per set;
                            // they are removed with the module.
                            pluginCount += set.plugins.length;
                            if (setElements && setElements.enableButton) {
                                setElements.enableButton.hide();
                            }
                            continue;
                        }
                        ...
                        for (var i=0;set.types && i<set.types.length;i++) {
                            var t = set.types[i];
                ...
                if (typeCount > 0) {
                    ...
                    countParts.push(RED._('palette.editor.nodeCount',{count:typeCount,label:nodeCount}));
                }
                if (pluginCount > 0) {
                    countParts.push(RED._('palette.editor.pluginCount',{count:pluginCount,label:pluginCount}));
                }
                nodeEntry.setCount.text(countParts.join(", "));

                if (moduleInfo.hasNodes) {
                    // Module provides node sets - show enable/disable all.
                    ...
                    // Plugin-only module: plugins cannot be disabled, only
                    // local modules can be removed.
                    nodeEntry.enableButton.hide();
                    nodeEntry.container.removeClass("disabled");
                    if (moduleInfo.local) {
                        nodeEntry.removeButton.css('display', 'inline-block');
                    } else {
                        nodeEntry.removeButton.hide();
    ...
    function init() {
        ...
        RED.events.on('registry:node-set-removed', function(ns) {
            ...
            if (!module || Object.keys(module.sets).length === 0) {
                ...
                // Node sets remain, or the module still provides plugins -
                // refresh the merged entry rather than dropping it.
                refreshNodeModule(ns.module);
            }
        });
            ...
            if (!nodeEntries.hasOwnProperty(module)) {
                ...
                for (var s in nodeEntries[module].info.sets) {
                    if (nodeEntries[module].info.sets.hasOwnProperty(s)) {
                        index.push(s);
                        var set = nodeEntries[module].info.sets[s];
                        if (set.types) {
                            index = index.concat(set.types);
                        }
                        if (set.plugins) {
        ...
        RED.events.on("registry:plugin-removed", function(ev) {
            var module = ev.module;
            if (module && nodeEntries[module]) {
                // A plugin set was removed but the module may still provide
                // other plugins or node sets - refresh the merged entry.
                _refreshNodeModule(module);
            }
        });
        RED.events.on("registry:plugin-module-removed", function(module) {
            if (getModuleInfo(module)) {
                // The module still provides node sets - refresh the entry.
                _refreshNodeModule(module);
                return;
            }
            ...
            addItem: function(container,i,object) {
                ...
                if (entry) {
                    ...
                    const populateSetList = function () {
                        var setList = Object.keys(object.info.sets)
                        setList.sort(function(A,B) {
                            return A.toLowerCase().localeCompare(B.toLowerCase());
                        });
                        setList.forEach(function(setName) {
                            var set = object.info.sets[setName];
                    ...
                    click: function() {
                        ...
                        removeNodeModule(entry.name, function(xhr) {
                            ...
                            if (xhr) {
                                ...
                                // Give any plugins in the module a chance to
                                // clean up their editor state. The list entry
                                // itself is removed via the node/removed
                                // notification; for a plugin-only module that
                                // notification carries the plugin sets.
                                var currentEntry = nodeEntries[entry.name];
                                var currentInfo = (currentEntry && currentEntry.info) ? currentEntry.info : entry;
                                if (currentInfo.hasPlugins) {
                                    let found_onremove = true;

                                    let keys = Object.keys(currentInfo.sets);
                                    keys.forEach((key) => {
                                        let set = currentInfo.sets[key];
                                    ...
                                    if (!currentInfo.hasNodes) {
                                        // Plugin-only module: there is no node
                                        // set removal to drive the list update,
                                        // so remove its entry directly.
                                        if (currentEntry) {
                                            nodeList.editableList('removeItem', currentEntry);
